...
func (l *ListCustomerLogic) ListCustomer(in *pb.ListUserRequest) (*pb.PagUserOut, error) {
	...
	if in.IsStaff != nil {
		query["is_staff = ?"] = in.IsStaff
	}
	if in.RoleId > 0 {
		query["role_id = ?"] = in.RoleId
	}
	...
	count, ms, err := l.svcCtx.User.ListModel(l.ctx, qp)
	if err != nil {
		return nil, database.NewGormError(err, nil)
	}
	mso := converter.ListUserModelToOut(ms)
	return &pb.PagUserOut{
		Items: mso,
		Page:  int64(page),
