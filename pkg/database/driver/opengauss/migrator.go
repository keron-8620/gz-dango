		...
		if field := stmt.Schema.LookUpField(field); field != nil {
			...
			if v, _ := fieldColumnType.DefaultValue(); v != field.DefaultValue {
				if field.HasDefaultValue && (field.DefaultValueInterface != nil || field.DefaultValue != "") {
					defaultValue := m.convertDefaultValue(field.DefaultValue)
					if field.DefaultValueInterface != nil {
						...
						if err := m.DB.Exec("ALTER TABLE ? ALTER COLUMN ? SET DEFAULT ?", m.CurrentTable(stmt), clause.Column{Name: field.DBName}, clause.Expr{SQL: defaultValue}).Error; err != nil {
							return err
						}
					} else {
						if err := m.DB.Exec("ALTER TABLE ? ALTER COLUMN ? DROP DEFAULT", m.CurrentTable(stmt), clause.Column{Name: field.DBName}, clause.Expr{SQL: defaultValue}).Error; err != nil {
							return err
						}
					}
				}
...
func (m Migrator) FullDataTypeOf(field *schema.Field) (expr clause.Expr) {
	...
	if field.HasDefaultValue && (field.DefaultValueInterface != nil || field.DefaultValue != "") {
		...
		} else if defaultValue := field.DefaultValue; defaultValue != "(-)" {
			defaultValue = m.convertDefaultValue(defaultValue)
			expr.SQL += " DEFAULT " + defaultValue
		}
	}
...
func (m Migrator) convertDefaultValue(dv string) string {
	switch strings.ToLower(dv) {
	case "now(3)": // 兼容 MySQL 的时间默认值。
